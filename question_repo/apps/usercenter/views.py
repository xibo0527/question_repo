...
from apps.repo.models import Answers,QuestionsCollection,Questions
    ...
    def post(self, request):
        ...
        if new_password1 != new_password2:
            ...
            if user:
                ...
                ret_info = {"code":200, "msg":"修改成功,请刷新页面重新登录"}
            else:
...
class CollectView(LoginRequiredMixin,ListView):
    context_object_name = 'my_collects'
    template_name = 'uc_collect.html'
    def get_queryset(self):
        return QuestionsCollection.objects.filter(user=self.request.user,status=1)
