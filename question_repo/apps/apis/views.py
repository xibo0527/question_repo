...
from apps.repo.models import Questions,Answers,QuestionsCollection,AnswersCollection,UserLog
...
class QuestionsView(View):
    def get(self, request):
        ...
        # 取出所有数据，筛选指定等级和分类 这里是status是题目的审核状态
...
class AnswerView(LoginRequiredMixin, View):
    def get(self, request, id):
        ...
        try:
            ...
            # {'id': 1, 'grade': None, 'category_id': None, 'title': '单向链表如何使用快速排序算法进行排序；', 'content': '', 'answer': '',
            #  'contributor_id': None, 'pub_time': None, 'status': True}
        except Exception as ex:
            print(ex)
            question = {}
        return JsonResponse(question, safe=False)
