...
class Questions(models.Model):
    ...
    grade = models.IntegerField(verbose_name="题目难度", choices=DIF_CHOICES, validators=[valid_difficulty], null=True)
    category = models.ForeignKey(Category, verbose_name="所属分类", null=True)
    title = models.CharField(verbose_name="题目标题", unique=True, max_length=256)
    # 富文本编辑器
    # content = models.TextField(verbose_name="题目详情", null=True)
    content = RichTextUploadingField(verbose_name="题目详情", null=True)
    # 富文本编辑器
    # answer = models.TextField(verbose_name="参考答案", null=True, blank=True)
    answer = RichTextUploadingField(verbose_name="参考答案", null=True, blank=True)
    contributor = models.ForeignKey(User, verbose_name="贡献者", null=True)
    pub_time = models.DateTimeField(verbose_name="入库时间", auto_now_add=True, null=True)
    # 审核状态
    ...
    class Meta:
        ...
        permissions = (
                       ...
                       ('can_add_question', "可以添加题目信息"),
                        ('can_change_question_status','可以修改审核状态')
                       )
...
class AnswersManager(models.Manager):
    def hot_question(self):
        ...
        question = self.all().values('question__title','question__grade','question__id').annotate(Count('id')).order_by('-id__count')[:5]
        return question
